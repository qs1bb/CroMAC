...
from envs.traffic.traffic_junction_env import *
...
class _LbfWrapper(MultiAgentEnv):
    def __init__(self, key, time_limit, pretrained_wrapper, **kwargs):
        ...
        self.last_action = np.zeros((self.n_agents,))
...
class _TrafficWrapper(MultiAgentEnv):
    def __init__(self, key, time_limit, pretrained_wrapper, **kwargs):
        self.episode_limit = time_limit
        difficulty = key.split(":")[1]
        if difficulty == "easy_slow":
            env = TrafficJunctionEnv("easy", 5, 0.3)
        else:
            env = TrafficJunctionEnv("easy", 4, 0.4)
...
REGISTRY["lbf"] = partial(env_fn, env=_LbfWrapper)
REGISTRY["traffic"] = partial(env_fn, env=_TrafficWrapper)
